...
if ($mode == 'queue') {
    ...
    if (is_array($queue_orders)) {
        ...
        fn_epayco_adaptive_set_summa_data($queue_orders, $orders_data);
        ...
        Tygh::$app['view']->assign('script_proceed', fn_url("payment_notification.pay?payment=epayco_adaptive&order_ids=$order_ids"));
        Tygh::$app['view']->assign('script_cancel', fn_url("payment_notification.cancel?payment=epayco_adaptive&order_ids={$parent_order_id}"));
    ...
    if (!empty($request['payment_id']) && !empty($request['order_ids'])) {
        ...
        $processor_data = fn_epayco_adaptive_get_processor_data($request['payment_id']);
        ...
        $result = fn_epayco_adaptive_request($request['order_ids'], $processor_data, $params);
