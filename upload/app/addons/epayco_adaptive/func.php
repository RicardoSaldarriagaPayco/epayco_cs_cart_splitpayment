...
{
    db_query("DELETE FROM ?:payment_processors WHERE processor_script = ?s", "epayco_adaptive.php");

    $_data = array(
        'processor' => 'Epayco Adaptive',
        'processor_script' => 'epayco_adaptive.php',
        'processor_template' => 'addons/epayco_adaptive/views/orders/components/payments/epayco_adaptive.tpl',
        'admin_template' => 'epayco_adaptive.tpl',
        'callback' => 'Y',
        'type' => 'P',
        'addon' => 'epayco_adaptive',
    );
...
function fn_epayco_adaptive_update_company_pre(&$company_data, $company_id, $lang_code, &$can_update)
{
    $company_data['company_id'] = $company_id;
    fn_epayco_adaptive_get_verified_status($company_data);
...
function fn_epayco_create_queue($params)
{
    ...
    $child_order_ids = fn_epayco_adaptive_get_childs_orders($params['order_id']);
...
function fn_epayco_adaptive_set_summa_data(&$queue_orders, $orders_data)
{
...
function fn_epayco_adaptive_get_childs_orders($order_id)
{
    static $child_orders;
...
function fn_epayco_adaptive_check_finish($parent_order_id)
{
...
function fn_epayco_adaptive_set_status($order_id, $status)
{
...
function fn_epayco_adaptive_any_paid($queue_orders)
{
    foreach ($queue_orders as $queue) {
...
function fn_epayco_adaptive_payment_refund($transaction_id, $processor_data)
{
}

function fn_epayco_adaptive_build_headers()
{
}

function fn_epayco_adaptive_payment_details($pay_key, $processor_data, $type_key = 'payKey')
{
}

function fn_epayco_adaptive_refund_status_message($refund_status)
{
    $statuses = array(
...
function fn_epayco_adaptive_get_processor_data($payment_id = 0)
{
    if (empty($payment_id)) {
        $processor_id = db_get_field("SELECT processor_id FROM ?:payment_processors WHERE processor_script = 'epayco_adaptive.php'");
 ...
 * @return array
 */
function fn_epayco_adaptive_request($order_ids = array(), $processor_data = array(), $params = array())
{
    $order_ids = array_diff($order_ids, array(''));

    $post_data = fn_epayco_adaptive_build_post($order_ids, $processor_data);
    ...
    $p_url_response = fn_url("payment_notification.response?payment=epayco_adaptive&order_id=$order_id", AREA, 'current');
    $p_url_confirmation = fn_url("payment_notification.confirmation?payment=epayco_adaptive&order_id=$order_id", AREA, 'current');
 ...
 * @return array
 */
function fn_epayco_adaptive_build_post($order_ids, $processor_data)
{
    $ip = fn_get_ip();
    ...
    $collect_payouts = Registry::get('addons.epayco_adaptive.collect_payouts') == 'Y';
    ...
    foreach ($order_ids as $order_id) {
        ...
        $admin_fee = fn_epayco_adaptive_calc_admin_fee($order_data);
 ...
 * @return float|int
 */
function fn_epayco_adaptive_calc_admin_fee($order_data)
{
    if (fn_epayco_adaptive_get_company_base_for_commission($order_data['company_id']) == 'O') {
    ...
    if (Registry::get('addons.epayco_adaptive.collect_payouts') == 'Y') {
 ...
 */
function fn_epayco_adaptive_get_company_base_for_commission($company_id)
{
    ...
    if (empty($base_for_commission)) {
        $base_for_commission = Registry::get('addons.epayco_adaptive.count_vendor_commission_on_basis');
    }
 ...
 */
function fn_epayco_adaptive_get_payments($params, $fields, $join, $order, &$condition, $having)
{
 ...
 */
function fn_update_epayco_adaptive_settings($settings)
{
    ...
    fn_attach_image_pairs('epayco_adaptive_logo', 'epayco_adaptive_logo', $company_id);
 ...
 * @return array mixed
 */
function fn_get_epayco_adaptive_settings($lang_code = DESCR_SL)
{
    $ppa_settings = Settings::instance()->getValues('epayco_adaptive', 'ADDON');

    $company_id = Registry::get('runtime.company_id');

    $ppa_settings['general']['main_pair'] = fn_get_image_pairs($company_id, 'epayco_adaptive_logo', 'M', false, true, $lang_code);
 ...
 */
function fn_epayco_adaptive_set_options($params)
{
    if (!empty($params['pay_key'])
        ...
        $post_data = fn_epayco_adaptive_set_options_build_post($params);
 ...
 * @return array
 */
function fn_epayco_adaptive_set_options_build_post($params)
{
    $post_data = array();
    ...
    if (!empty($params['pay_key'])) {
        ...
        $company_id = Registry::get('runtime.company_id');
        $header_image = fn_get_image_pairs($company_id, 'epayco_adaptive_logo', 'M', false, true);
...
function fn_epayco_adaptive_get_companies($params, &$fields, $sortings, $condition, $join, $auth, $lang_code, $group)
{
...
function fn_epayco_adaptive_get_verified_status($company_data)
{
}

function fn_sd_epayco_adaptive_cs_info_url_auto_epayco_verification()
{
}

function fn_epayco_adaptive_gather_additional_product_data_before_options(&$product, $auth, $params)
{
...
function fn_epayco_adaptive_calculate_commission($order_info, $company_data, $payout_data)
{
    return fn_calculate_commission_for_payout($order_info, $company_data, $payout_data);
}

function fn_epayco_adaptive_mve_place_order(&$order_info, &$company_data, &$action, &$order_status, &$cart, &$data, &$payout_id, &$auth)
{
    list($is_processor_script, ) = fn_check_processor_script($order_info['payment_id']);
    if ($is_processor_script && fn_check_payment_script('epayco_adaptive.php', $order_info['order_id'])) {
        $data = fn_epayco_adaptive_calculate_commission($order_info, $company_data, $data);
    }
}

function fn_epayco_adaptive_mve_update_order(&$new_order_info, &$order_id, &$old_order_info, &$company_data, &$payout_id, &$payout_data)
{
    list($is_processor_script, ) = fn_check_processor_script($new_order_info['payment_id']);
    if ($is_processor_script && fn_check_payment_script('epayco_adaptive.php', $old_order_info['order_id'])) {
        $payout_data = fn_epayco_adaptive_calculate_commission($new_order_info, $company_data, $payout_data);
    }
}
 ...
 */
function fn_epayco_adaptive_vendor_payouts_get_list(&$instance, &$params, &$items_per_page, &$fields, &$join, &$condition, &$date_condition, &$sorting, &$limit)
{
 ...
 * @param string        $date_condition Additional condition to filter payouts by date
 */
function fn_epayco_adaptive_vendor_payouts_get_income(&$instance, &$params, &$fields, &$join, &$condition, &$date_condition)
{
    if ($instance->getVendor()) {
