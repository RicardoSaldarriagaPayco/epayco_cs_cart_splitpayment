{if "MULTIVENDOR"|fn_allowed_for && ($company_name || $company_id) && $settings.Vendors.display_vendor == "Y"}
    <div class="ty-control-group{if !$capture_options_vs_qty} product-list-field{/if} epayco-adaptive-vendor-name {if !empty($product.epayco_verification.verified) && $product.epayco_verification.verified == "verified"}epayco-adaptive-vendor-name-text{/if}">
