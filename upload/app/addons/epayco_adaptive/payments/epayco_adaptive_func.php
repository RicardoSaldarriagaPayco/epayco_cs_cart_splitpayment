...
function fn_epayco_adaptive_next($order_ids)
{
    $parent_order_id = fn_epayco_get_parent_order_id($order_ids);

    if (fn_epayco_adaptive_check_finish($parent_order_id)) {
        ...
        $action = "epayco_adaptive.queue?order_id=$parent_order_id";
        fn_redirect(fn_url($action));
    }
}

function fn_epayco_adaptive_start_payments($order_ids)
{
    $idata = array();
...
function fn_epayco_adaptive_check_payment_complete($payment_details, $order_index)
{
...
function fn_epayco_adaptive_check_payment_pending($payment_details, $order_index)
{
