...
include_once (Registry::get('config.dir.addons') . 'epayco_adaptive/payments/epayco_adaptive_func.php');
...
if (defined('PAYMENT_NOTIFICATION')) {
        ...
        if(empty($validationData)){
            ...
            if(!empty($order_id) && !empty($validationData)){
                ...
                if($signature == $x_signature && $validation){
                    ...
                    if (fn_check_payment_script('epayco_adaptive.php', $order_id)) {
    ...
    $processor_data = fn_epayco_adaptive_get_processor_data($payment_id);
    ...
    if ($mode == 'return') {
        ...
        $payment_details = fn_epayco_adaptive_payment_details($pay_key, $processor_data);
        $queue_orders = fn_epayco_get_queue($parent_order_id);

        if (!empty($queue_orders) && !fn_epayco_adaptive_any_paid($queue_orders)) {
        ...
        if (isset($processor_data['processor_params']['in_context'])
            ...
            fn_epayco_adaptive_start_payments($order_ids);
        }
        ...
        foreach($order_ids as $order_index => $order_id) {
            ...
            if (fn_epayco_adaptive_check_payment_complete($payment_details, $order_index)) {
                $pp_response['order_status'] = 'P';
                $pp_response['reason_text'] = 'Success';

                $create_withdrawal = $create_withdrawal && true;

            } elseif (fn_epayco_adaptive_check_payment_pending($payment_details, $order_index)) {
                $pp_response['order_status'] = 'O';
            ...
            if ($create_withdrawal) {
                ...
                $payouts_manager->update(array(
                    ...
                    'comments' => __('addons.epayco_adaptive.withdrawal_for_the_order', array(
                        '[order_id]' => $order_id,
                    )),
                ));
            ...
            fn_finish_payment($order_id, $pp_response);
            fn_epayco_adaptive_set_status($order_id, PAID_PAYMENT_ORDERS);
        }

        fn_epayco_adaptive_next($order_ids);
        ...
        fn_epayco_adaptive_start_payments($order_ids);
        fn_epayco_adaptive_request($order_ids, $processor_data);
    }
    ...
    fn_epayco_adaptive_request(array($order_id), $processor_data);
}